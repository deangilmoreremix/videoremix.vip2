...
class EventQueue {
  ...
  private static async sendToAnalytics(events: AnalyticsEvent[]): Promise<void> {
    try {
      ...
      // Store in localStorage as fallback for offline functionality
      const existing = JSON.parse(localStorage.getItem('analytics_queue') || '[]');
      const updatedQueue = [...existing, ...events];
      localStorage.setItem('analytics_queue', JSON.stringify(updatedQueue));

      // Try to send queued events when back online
      if (navigator.onLine) {
        setTimeout(() => this.flushQueuedEvents(), 1000);
      }
  ...
  private static async flushQueuedEvents(): Promise<void> {
    const queuedEvents = JSON.parse(localStorage.getItem('analytics_queue') || '[]');
    if (queuedEvents.length === 0) return;

    try {
      await this.sendToAnalytics(queuedEvents);
      localStorage.removeItem('analytics_queue');
      console.log(`✅ Flushed ${queuedEvents.length} queued analytics events`);
    } catch (error) {
      console.warn('Failed to flush queued analytics events:', error);
    }
  }
...
// Performance monitoring is handled by the existing PerformanceMonitor class above
