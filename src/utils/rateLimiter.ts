...
class RedisClientFactory {
  ...
  static async getClient(): Promise<RedisClient | null> {
    ...
    try {
      ...
      console.warn("Failed to connect to Redis, falling back to in-memory rate limiting:", error.message);
      return null;
    }
  }
...
class RateLimiter {
  ...
  async checkLimit(identifier: string, action: string = 'default'): Promise<{ allowed: boolean; remaining: number; resetTime: number }> {
  ...
  private checkLimitMemory(key: string, now: number): { allowed: boolean; remaining: number; resetTime: number } {
  ...
  async checkAdminLimit(userId: string, action: string = 'default'): Promise<{ allowed: boolean; remaining: number; resetTime: number }> {
    ...
    if (this.useRedis && this.redisClient) {
      return this.checkLimitRedis(key, Date.now(), limit.MAX_REQUESTS, limit.WINDOW_MS);
    } else {
      return this.checkAdminLimitMemory(key, Date.now(), limit);
    }
  }

  private checkAdminLimitMemory(key: string, now: number, limit: { WINDOW_MS: number; MAX_REQUESTS: number }): { allowed: boolean; remaining: number; resetTime: number } {
    const entry = this.limits.get(key);
  ...
  private async checkLimitRedis(key: string, now: number, maxRequests: number, windowMs: number): Promise<{ allowed: boolean; remaining: number; resetTime: number }> {
    if (!this.redisClient) {
    ...
    try {
      ...
      if (!countStr || now > resetTime) {
        ...
        await Promise.all([
          this.redisClient.set(countKey, '1', { ex: Math.ceil(windowMs / 1000) }),
          this.redisClient.set(resetKey, newResetTime.toString(), { ex: Math.ceil(windowMs / 1000) })
        ]);
      ...
      if (currentCount >= maxRequests) {
        console.warn(`RateLimiter: Rate limit exceeded for ${key}, remaining: ${Math.ceil((resetTime - now) / 1000)}s`);
        return {
          allowed: false,
          remaining: 0,
          resetTime: resetTime
...
export async function checkAdminRateLimit(userId: string, action: string = 'default'): Promise<{ allowed: boolean; error?: string }> {
