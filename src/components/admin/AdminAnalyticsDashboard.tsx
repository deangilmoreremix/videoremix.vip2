...
import { TrendingUp, TrendingDown, DollarSign, Users, ShoppingCart, Activity, BarChart3, PieChart, Calendar } from 'lucide-react';
import { motion } from 'framer-motion';

interface AnalyticsData {
  totalApps: { count: number; active: number; inactive: number };
  users: { count: number; growth: string; regular: number; admins: number; newThisMonth: number };
  purchases: { total: number; recent: number; revenue: string; recentRevenue: string };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm font-medium text-blue-400">{analytics.users.growth}</span>
          </div>
          <div className="text-3xl font-bold text-white mb-1">{analytics.users.count.toLocaleString()}</div>
        ...
        >
          ...
          <div className="text-3xl font-bold text-white mb-1">${parseFloat(analytics.purchases.revenue).toLocaleString()}</div>
          <div className="text-sm text-gray-400">Total Revenue</div>
          <div className="mt-4 pt-4 border-t border-green-500/20 text-xs text-gray-400">
            ${parseFloat(analytics.purchases.recentRevenue).toLocaleString()} recent
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="text-3xl font-bold text-white mb-1">{analytics.purchases.total.toLocaleString()}</div>
        ...
        >
          ...
          <div className="text-3xl font-bold text-white mb-1">{analytics.appAccess.total.toLocaleString()}</div>
        ...
        <>
          {analytics.metrics && (
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div>
                  <div className="text-2xl font-bold text-blue-400">${analytics.metrics.avgRevenuePerUser}</div>
                  <div className="text-sm text-gray-400">Avg Revenue Per User</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-green-400">${analytics.metrics.avgRevenuePerPurchase}</div>
                  <div className="text-sm text-gray-400">Avg Purchase Value</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-purple-400">{analytics.metrics.subscriptionRetentionRate}%</div>
                  <div className="text-sm text-gray-400">Subscription Retention</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-orange-400">{analytics.metrics.avgAppsPerUser}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
              ...
              <div className="space-y-3">
                {analytics.analytics.topApps.slice(0, 5).map((app, index) => (
                  <div key={app.app_slug} className="flex items-center justify-between">
                    <div className="flex items-center">
                      <span className="text-gray-400 font-medium mr-3">{index + 1}</span>
                      <span className="text-white">{app.app_slug}</span>
                    </div>
                    <span className="text-blue-400 font-semibold">{app.user_count} users</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-3">
                {analytics.analytics.topProducts.slice(0, 5).map((product, index) => (
                  <div key={product.name} className="flex items-center justify-between">
                    ...
                    <div className="text-right ml-4">
                      <div className="text-green-400 font-semibold">${product.revenue.toFixed(2)}</div>
                      <div className="text-xs text-gray-400">{product.count} sales</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          >
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {analytics.analytics.dailyTrends.slice(0, 7).map((trend, index) => (
                    <tr key={trend.date} className="border-b border-gray-800 text-sm">
                      <td className="py-3 text-gray-300">{new Date(trend.date).toLocaleDateString()}</td>
                      <td className="py-3 text-white">{trend.users.toLocaleString()}</td>
                      <td className="py-3 text-blue-400">{trend.activeUsers.toLocaleString()}</td>
                      <td className="py-3 text-green-400">+{trend.newUsers}</td>
                      <td className="py-3 text-purple-400">{trend.purchases}</td>
                      <td className="py-3 text-green-400">${trend.revenue.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
              ...
              <div className="space-y-3">
                {Object.entries(analytics.analytics.revenueByPlatform).map(([platform, data]) => (
            ...
            <motion.div
              ...
              className="bg-gray-800/70 backdrop-blur-sm rounded-xl p-6 border border-gray-700"
            >
              <h3 className="text-lg font-semibold text-white mb-4">Apps by Category</h3>
              <div className="space-y-3">
                {Object.entries(analytics.analytics.categoryBreakdown).map(([category, data]) => (
                  <div key={category} className="flex items-center justify-between">
                    <span className="text-white">{category}</span>
                    <div className="text-right">
                      <span className="text-blue-400 font-semibold">{data.active}</span>
