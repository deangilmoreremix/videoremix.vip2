...
async function createAllMissingUsers() {
  console.log('🚀 Creating ALL remaining missing users...\n');
  ...
  // Create users in smaller batches to avoid rate limits
  ...
  for (let i = 0; i < usersToCreate.length; i += BATCH_SIZE) {
    ...
    console.log(`Batch ${batchNum}: Processing ${batch.length} users (${i + 1}-${Math.min(i + BATCH_SIZE, usersToCreate.length)}/${usersToCreate.length})`);
      ...
      try {
        ...
        if (error) {
          ...
          try {
            ...
            console.error(`⚠️  Profile error for ${userData.email}:`, profileErr.message);
          }
        }
      } catch (error) {
        console.error(`❌ Exception creating ${userData.email}:`, error.message);
        failed++;
        errors.push({ email: userData.email, error: error.message });
      }
    }
    ...
    if (batchNum % 10 === 0) {
      console.log(`📊 Progress: ${Math.min(i + BATCH_SIZE, usersToCreate.length)}/${usersToCreate.length} processed (${created} created, ${failed} failed)`);
    }
  ...
  if (errors.length > 0) {
    ...
    errors.slice(0, 10).forEach(err => console.log(`  ${err.email}: ${err.error}`));
    if (errors.length > 10) {
      console.log(`  ... and ${errors.length - 10} more errors`);
    }
  }

  console.log(`\n🎯 Expected final auth users: ${existingEmails.size + created}`);
}

createAllMissingUsers().catch(console.error);