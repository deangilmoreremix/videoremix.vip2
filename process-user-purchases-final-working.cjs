  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      if (productName && productName !== 'nan' && productName.trim() !== '') {
        // Remove surrounding quotes if present (though parseCSVLine should have removed them)
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: node process-user-purchases-final-working.cjs <csv-file-path>');
