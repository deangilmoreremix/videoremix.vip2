...
async function createNext500Users() {
  console.log('👥 Starting Next 500 Users Import Process...\n');

  // Read the next 500 users text file
  const content = readFileSync('./next_500_users.txt', 'utf-8');
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    for (let userIndex = 0; userIndex < batch.length; userIndex++) {
      const line = batch[userIndex].trim();
      if (!line) continue;

      const globalLineNum = batchIndex * batchSize + userIndex + 1;
      ...
      try {
        ...
        const { data: newUser, error: createError } = await supabase.auth.admin.createUser({
          ...
          user_metadata: {
            ...
            last_name: lastName,
            created_via: 'next_500_users_import',
          },
        });
  ...
  // Print summary
  console.log('\n' + '='.repeat(60));
  console.log('👥 NEXT 500 USERS IMPORT SUMMARY');
  ...
  console.log('\n✅ Next 500 users import process completed!\n');
}

// Run the script
createNext500Users().catch(console.error);