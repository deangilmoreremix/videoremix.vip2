...
interface OrchestrationInput {
  ...
  _city: string;
  _stateprovince_optional: string;
  _minimum_price_: string;
  _maximum_price_: string;
  _property_type: string;
  _bedrooms: string;
  _bathrooms: string;
  _minimum_square_feet: string;
  _timeline: string;
  _urgency: string;
  _special_features__requirements: string
  userId?: string;
}
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-real-estate-agent-team',
