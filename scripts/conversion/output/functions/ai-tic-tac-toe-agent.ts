...
function buildPrompt(input: AgentInput): string {
  return `Process the following: {primaryInput}`;
}
...
export async function handler(event: any) {
  ...
  const resultId = `ai-tic-tac-toe-agent_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-tic-tac-toe-agent',
