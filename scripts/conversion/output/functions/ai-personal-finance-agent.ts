...
interface OrchestrationInput {
  enter_openai_api_key_to_access_gpt4o: string;
  enter_serp_api_key_for_search_functionality: string;
  what_are_your_financial_goals: string;
  describe_your_current_financial_situation: string
  userId?: string;
}
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-personal-finance-agent',
