...
interface AgentInput {
  ...
  model: string;
  upload_pdfs_or_text_files?: string;
...
Model: {model}
Upload PDFs or text files: {upload_pdfs_or_text_files}
...
export async function handler(event: any) {
  ...
  if (!input.model) return { statusCode: 400, body: JSON.stringify({ error: 'Model is required' }) };
  if (!input.upload_pdfs_or_text_files) return { statusCode: 400, body: JSON.stringify({ error: 'Upload PDFs or text files is required' }) };
  if (!input.research_question) return { statusCode: 400, body: JSON.stringify({ error: 'Research question is required' }) };

  const resultId = `ag2-adaptive-research-team_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ag2-adaptive-research-team',
