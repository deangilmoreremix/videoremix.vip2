...
function buildPrompt(input: AgentInput): string {
  return `🔑 Google API Key: {_google_api_key}
📝 Research Goal: {_research_goal}`;
}
...
export async function handler(event: any) {
  ...
  if (!input._google_api_key) return { statusCode: 400, body: JSON.stringify({ error: '🔑 Google API Key is required' }) };
  if (!input._research_goal) return { statusCode: 400, body: JSON.stringify({ error: '📝 Research Goal is required' }) };

  const resultId = `research-agent-gemini-interaction-api_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'research-agent-gemini-interaction-api',
