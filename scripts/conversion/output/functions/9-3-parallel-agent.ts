...
function buildPrompt(input: AgentInput): string {
  return `Research topic: {research_topic}`;
}
...
export async function handler(event: any) {
  ...
  if (!input.research_topic) return { statusCode: 400, body: JSON.stringify({ error: 'Research topic is required' }) };

  const resultId = `9-3-parallel-agent_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
