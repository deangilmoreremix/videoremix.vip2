...
interface OrchestrationInput {
  how_have_you_been_feeling_recently: string;
  current_stress_level_110: string;
  current_support_system: string;
  any_significant_life_changes_or_events_recently: string;
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-mental-wellbeing-agent',
