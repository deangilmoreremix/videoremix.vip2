...
interface OrchestrationInput {
  ...
  how_would_you_like_to_enter_your_expenses: string;
  choose_your_csv_file?: string;
  how_many_debts_do_you_have: string;
  name: string;
  amount_: string;
  interest_rate_: string;
  minimum_payment_: string
  userId?: string;
}
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-financial-coach-agent',
