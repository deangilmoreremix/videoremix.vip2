...
function buildPrompt(input: AgentInput): string {
  return `📤 Upload Video: {_upload_video}`;
}
...
export async function handler(event: any) {
  ...
  if (!input._upload_video) return { statusCode: 400, body: JSON.stringify({ error: '📤 Upload Video is required' }) };

  const resultId = `frontend_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
