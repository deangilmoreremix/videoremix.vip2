...
interface OrchestrationInput {
  enter_your_gemini_api_key: string;
  upload_uiux_designs?: string;
  upload_competitor_designs_optional?: string;
  select_analysis_types: string;
  focus_areas: string;
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'multimodal-design-agent-team',
