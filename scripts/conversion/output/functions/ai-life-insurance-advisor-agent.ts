...
interface OrchestrationInput {
  ...
  dependents: string;
  country__state: string;
  total_outstanding_debt_incl_mortgage: string;
  savings__investments_available_to_dependents: string;
  existing_life_insurance: string;
  currency: string;
  income_replacement_horizon: string
  userId?: string;
}
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-life-insurance-advisor-agent',
