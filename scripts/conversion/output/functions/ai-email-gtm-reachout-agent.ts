...
interface OrchestrationInput {
  what_type_of_companies_should_we_target: string;
  preferred_company_size: string;
  number_of_companies_to_find: string;
  your_name_: string;
  your_email_: string;
  your_organization_: string;
  linkedin_profile_optional: string;
  phone_number_optional: string;
  company_website_optional: string;
  calendar_link_optional: string;
  describe_your_offering_: string;
  serviceproduct_category: string;
  which_departments_should_we_target: string;
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-email-gtm-reachout-agent',
