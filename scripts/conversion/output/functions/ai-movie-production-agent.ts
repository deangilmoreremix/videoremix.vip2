...
interface OrchestrationInput {
  enter_google_api_key_to_access_gemini_25_flash: string;
  enter_serp_api_key_for_search_functionality: string;
  describe_your_movie_idea_in_a_few_sentences: string;
  select_the_movie_genre: string;
  select_the_target_audience: string;
  estimated_runtime_in_minutes: string
  userId?: string;
}
...
export async function handler(event: any) {
  ...
  try {
    ...
    await supabase.from('ai_agent_runs').insert({
      id: resultId,
      agent_type: 'ai-movie-production-agent',
