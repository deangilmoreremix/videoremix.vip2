...
const AgenticRagGpt5Page: React.FC = () => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({ openai_api_key: "", add_url: "", your_question: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/agentic-rag-gpt5', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        <title>AgenticRagGpt5 - VideoRemix.vip</title>
        <meta name="description" content="Use agentic-rag-gpt5 to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4">Agentic Rag Gpt5</h1>
            <p className="text-xl text-gray-400">AI-powered agentic rag gpt5.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="add_url">Add URL *</Label>
                
                <Input
                  id="add_url"
                  type="text"
                  value={formData.add_url}
                  onChange={(e) => setFormData({ ...formData, add_url: e.target.value })}
                  placeholder=""
                  className="bg-gray-900/50 border-gray-600 text-white"
                />
