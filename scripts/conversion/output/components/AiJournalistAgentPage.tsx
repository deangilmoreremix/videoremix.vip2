...
const AiJournalistAgentPage: React.FC = () => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({ enter_openai_api_key_to_access_gpt4o: "", enter_serp_api_key_for_search_functionality: "", what_do_you_want_the_ai_journalist_to_write_an_article_on: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/ai-journalist-agent', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Use ai-journalist-agent to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4">Ai Journalist Agent</h1>
            <p className="text-xl text-gray-400">AI-powered ai journalist agent.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="what_do_you_want_the_ai_journalist_to_write_an_article_on">What do you want the AI journalist to write an Article on? *</Label>
                
                <Input
                  id="what_do_you_want_the_ai_journalist_to_write_an_article_on"
                  type="text"
                  value={formData.what_do_you_want_the_ai_journalist_to_write_an_article_on}
                  onChange={(e) => setFormData({ ...formData, what_do_you_want_the_ai_journalist_to_write_an_article_on: e.target.value })}
                  placeholder=""
                  className="bg-gray-900/50 border-gray-600 text-white"
                />
