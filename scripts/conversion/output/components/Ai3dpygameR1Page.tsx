...
const Ai3dpygameR1Page: React.FC = () => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({ deepseek_api_key: "", openai_api_key: "", enter_your_pygame_query: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/ai-3dpygame-r1', {
        method: 'POST',
      ...
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            ...
            <p className="text-xl text-gray-400">AI-powered ai 3dpygame r1.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="enter_your_pygame_query">Enter your PyGame query: *</Label>
                
                <Textarea
                  id="enter_your_pygame_query"
                  value={formData.enter_your_pygame_query}
                  onChange={(e) => setFormData({ ...formData, enter_your_pygame_query: e.target.value })}
                  placeholder=""
