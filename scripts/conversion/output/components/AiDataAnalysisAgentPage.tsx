...
const AiDataAnalysisAgentPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('upload_a_csv_or_excel_file', textValues.upload_a_csv_or_excel_file || '');
      formData.append('ask_a_query_about_the_data', textValues.ask_a_query_about_the_data || '');
      const res = await fetch('/.netlify/functions/ai-data-analysis-agent', {
        method: 'POST',
        body: formData
      });
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Use ai-data-analysis-agent to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            ...
            <p className="text-xl text-gray-400">AI-powered ai data analysis agent.</p>
          </motion.div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTextValues(prev => ({ ...prev, enter_your_openai_api_key: e.target.value }))}
                    placeholder=""
                ...
                <div className="space-y-2">
                  <Label htmlFor="upload_a_csv_or_excel_file">Upload a CSV or Excel file *</Label>
                  <Input
                    id="upload_a_csv_or_excel_file"
                    value={textValues.upload_a_csv_or_excel_file || ''}
                    onChange={(e) => setTextValues(prev => ({ ...prev, upload_a_csv_or_excel_file: e.target.value }))}
                    placeholder=""
                    className="bg-gray-900/50 border-gray-600"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="ask_a_query_about_the_data">Ask a query about the data: *</Label>
                  <Input
                    id="ask_a_query_about_the_data"
                    value={textValues.ask_a_query_about_the_data || ''}
                    onChange={(e) => setTextValues(prev => ({ ...prev, ask_a_query_about_the_data: e.target.value }))}
                    placeholder=""
                    className="bg-gray-900/50 border-gray-600"
                  />
                </div>
