...
const AgenticRagEmbeddingGemmaPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/agentic-rag-embedding-gemma', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        <title>AgenticRagEmbeddingGemma - VideoRemix.vip</title>
        <meta name="description" content="Use agentic-rag-embedding-gemma to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4">Agentic Rag Embedding Gemma</h1>
            <p className="text-xl text-gray-400">AI-powered agentic rag embedding gemma.</p>
          </motion.div>
