...
const AutonomousRagPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/autonomous-rag', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Use autonomous-rag to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4">Autonomous Rag</h1>
            <p className="text-xl text-gray-400">AI-powered autonomous rag.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="_ask_your_question">💬 Ask Your Question: *</Label>
                
                <Input
                  id="_ask_your_question"
                  type="text"
                  value={formData._ask_your_question}
                  onChange={(e) => setFormData({ ...formData, _ask_your_question: e.target.value })}
                  placeholder=""
