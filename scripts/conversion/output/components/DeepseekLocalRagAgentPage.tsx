...
const DeepseekLocalRagAgentPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/deepseek-local-rag-agent', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        <title>DeepseekLocalRagAgent - VideoRemix.vip</title>
        <meta name="description" content="Use deepseek-local-rag-agent to automate tasks with AI." />
      </Helmet>

      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 h-[calc(100vh-8rem)] flex flex-col max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
            <h1 className="text-3xl font-bold mb-2">Deepseek Local Rag Agent</h1>
            <p className="text-gray-400">AI-powered deepseek local rag agent.</p>
          </motion.div>
