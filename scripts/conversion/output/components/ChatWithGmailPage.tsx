...
const ChatWithGmailPage: React.FC = () => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({ enter_your_openai_api_key: "", ask_any_question_about_your_emails: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/chat-with-gmail', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Use chat-with-gmail to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4">Chat With Gmail</h1>
            <p className="text-xl text-gray-400">AI-powered chat with gmail.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="ask_any_question_about_your_emails">Ask any question about your emails *</Label>
                
                <Input
                  id="ask_any_question_about_your_emails"
                  type="text"
                  value={formData.ask_any_question_about_your_emails}
                  onChange={(e) => setFormData({ ...formData, ask_any_question_about_your_emails: e.target.value })}
                  placeholder=""
                  className="bg-gray-900/50 border-gray-600 text-white"
                />
