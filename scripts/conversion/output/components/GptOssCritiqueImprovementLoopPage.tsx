...
const GptOssCritiqueImprovementLoopPage: React.FC = () => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({ groq_api_key: "", max_improvement_iterations: "", your_prompt: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/gpt-oss-critique-improvement-loop', {
        method: 'POST',
      ...
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            ...
            <p className="text-xl text-gray-400">AI-powered gpt oss critique improvement loop.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="max_improvement_iterations">Max Improvement Iterations *</Label>
                
                <Input
                  id="max_improvement_iterations"
                  type="range"
                  value={formData.max_improvement_iterations}
                  onChange={(e) => setFormData({ ...formData, max_improvement_iterations: e.target.value })}
                  placeholder=""
                  className="bg-gray-900/50 border-gray-600 text-white"
                />
              ...
              <div className="space-y-2">
                <Label htmlFor="your_prompt">Your prompt *</Label>
                
                <Textarea
                  id="your_prompt"
                  value={formData.your_prompt}
                  onChange={(e) => setFormData({ ...formData, your_prompt: e.target.value })}
                  placeholder=""
