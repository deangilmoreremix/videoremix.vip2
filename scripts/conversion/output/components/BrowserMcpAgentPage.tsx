...
const BrowserMcpAgentPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/browser-mcp-agent', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Use browser-mcp-agent to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4">Browser Mcp Agent</h1>
            <p className="text-xl text-gray-400">AI-powered browser mcp agent.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="your_command">Your Command *</Label>
                
                <Textarea
                  id="your_command"
                  value={formData.your_command}
                  onChange={(e) => setFormData({ ...formData, your_command: e.target.value })}
                  placeholder=""
