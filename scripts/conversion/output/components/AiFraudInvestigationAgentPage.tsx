...
const AiFraudInvestigationAgentPage: React.FC = () => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({ openrouter_api_key: "", google_maps_api_key: "", model: "", zip_code_cook_county_il: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/.netlify/functions/ai-fraud-investigation-agent', {
        method: 'POST',
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Use ai-fraud-investigation-agent to automate tasks with AI." />
      </Helmet>
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4">Ai Fraud Investigation Agent</h1>
            <p className="text-xl text-gray-400">AI-powered ai fraud investigation agent.</p>
          </motion.div>
          ...
          <Card className="bg-gray-800/50 border-gray-700 mb-8">
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="zip_code_cook_county_il">ZIP Code (Cook County, IL) *</Label>
                
                <select
                  id="zip_code_cook_county_il"
                  value={formData.zip_code_cook_county_il}
                  onChange={(e) => setFormData({ ...formData, zip_code_cook_county_il: e.target.value })}
                  className="w-full bg-gray-900/50 border border-gray-600 rounded-md px-3 py-2 text-white"
                >
                  <option value=""></option>
                </select>
