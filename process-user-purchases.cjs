...
for (let i = 1; i < lines.length; i++) {
  ...
  try {
    ...
    console.log(`\nProcessing: ${email} (${fullName}) - Product: ${productName}`);
      ...
      if (productName) {
        ...
        if (!product) {
          ...
          if (purchases && purchases.length > 0) {
            console.log(`Found ${purchases.length} completed purchases for ${email}`);
            // Process each purchase
            for (const purchase of purchases) {
              if (!purchase.product_id) {
                console.warn(`Purchase ${purchase.id} has no product_id`);
                continue;
              }
              ...
              if (productError) {
                console.error(`Error fetching product for purchase ${purchase.id}:`, productError.message);
                continue;
              }

              if (!productData || !productData.apps_granted || productData.apps_granted.length === 0) {
                console.warn(`Product ${productData?.name} has no apps_granted`);
                continue;
              }

              // Grant access for each app in apps_granted
              for (const appSlug of productData.apps_granted) {
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: node process-user-purchases.cjs <csv-file-path>');
