...
async function processCsvFile(filePath) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      // We expect 8 columns, but due to commas in fields, we may have more
      // Fields that can contain commas: total_spend (index 3) and products_purchased (index 7)
      
      if (parts.length < 8) {
        console.warn(`Skipping row ${i + 1}: insufficient columns (${parts.length} < 8)`);
        continue;
      }
      
      // Reconstruct the fields accounting for commas in total_spend and products_purchased
      let reconstructed = [];
      
      // Add first 3 fields (first_name, last_name, email)
      for (let j = 0; j < 3; j++) {
        reconstructed.push(parts[j].trim());
      }
      
      // Handle total_spend (field index 3) - it may be split into two parts due to comma
      // If we have more than 8 parts, parts[3] and parts[4] belong to total_spend
      if (parts.length > 8) {
        reconstructed.push((parts[3] + ',' + parts[4]).trim()); // total_spend with comma
        // Products purchased starts at index 5, but we need to adjust for the extra part
        let productStart = 5;
        // Combine all remaining parts for products_purchased
        let productParts = [];
        for (let j = productStart; j < parts.length; j++) {
          productParts.push(parts[j]);
        }
        reconstructed.push(productParts.join(',').trim()); // products_purchased
      } else {
        // Normal case: exactly 8 parts
        reconstructed.push(parts[3].trim()); // total_spend
        reconstructed.push(parts[4].trim()); // total_orders
        reconstructed.push(parts[5].trim()); // last_purchase
        reconstructed.push(parts[6].trim()); // segment
        reconstructed.push(parts[7].trim()); // products_purchased
      }
      
      // Now we should have exactly 8 fields in reconstructed
      if (reconstructed.length !== 8) {
        console.warn(`Skipping row ${i + 1}: could not reconstruct to 8 fields, got ${reconstructed.length}`);
        continue;
      }
      
      const [firstName, lastName, email, totalSpend, totalOrders, lastPurchase, segment, productName] = reconstructed.map(f => f.trim());
      ...
      console.log(`\nProcessing: ${email} (${firstName} ${lastName})`);
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: node process-user-purchases-final-fix.cjs <csv-file-path>');
