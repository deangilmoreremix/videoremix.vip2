...
// Function to find best matching product using fuzzy matching
async function findBestProductMatch(csvProductName) {
  // First try exact match
  let exactMatch = await getProductByNameOrSku(csvProductName);
  if (exactMatch) {
    return { product: exactMatch, type: 'exact' };
  }
  ...
  if (error) {
    console.error(`Error fetching products for fuzzy matching:`, error.message);
    return null;
  }
  ...
  for (const product of products) {
    const score = similarity(csvProductName, product.name);
...
async function processCsvFile(filePath) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      const totalOrdersIndex = values.length - 4;
      const lastPurchaseIndex = values.length - 3;
      const segmentIndex = values.length - 2;
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: node process-user-purchases-mapped.cjs <csv-file-path>');
