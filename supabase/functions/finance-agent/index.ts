...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!action) {
      return new Response(JSON.stringify({ error: 'Action is required' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } });;
    }

    // Get stock data
    if (action === 'getStock') {
      if (!input.symbol) {
        return new Response(JSON.stringify({ error: 'Symbol is required' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } });;
      }
    ...
    if (action === 'analyze') {
      if (!input.stockData) {
        return new Response(JSON.stringify({ error: 'Stock data is required' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } });;
      }
      ...
      return new Response(JSON.stringify({ insight }), { status: 200, headers: { 'Content-Type': 'application/json', ...corsHeaders } });;
    }

    // Answer question
    if (action === 'ask') {
      if (!input.question || !input.stockData) {
        return new Response(JSON.stringify({ error: 'Question and stock data are required' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } });;
      }
