...
Write a self-contained HTML report to the OS temp directory (`$TMPDIR` on macOS/Linux, `%TEMP%` on Windows). Use Tailwind via CDN for layout and Mermaid via CDN for diagrams.

For each candidate, the card template:
...
- **Want to explore alternative interfaces?** Propose them.