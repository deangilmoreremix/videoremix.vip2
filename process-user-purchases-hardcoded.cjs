...
// Function to parse a line with known problematic comma in total_spend
function parseLine(line) {
  // Split by comma
  const parts = line.split(',');
  
  // We expect at least 8 fields, but total_spend may contain an extra comma
  if (parts.length < 8) {
  ...
  // First three fields: first_name, last_name, email
  reconstructed.push(parts[0].trim()); // first_name
  reconstructed.push(parts[1].trim()); // last_name
  reconstructed.push(parts[2].trim()); // email
  
  // Now handle total_spend and total_orders
  if (parts.length === 9) {
    // We have an extra comma, likely in total_spend
    // Combine parts[3] and parts[4] as total_spend
    reconstructed.push((parts[3] + ',' + parts[4]).trim()); // total_spend
    // Shift the remaining fields: total_orders is now parts[5], etc.
    reconstructed.push(parts[5].trim()); // total_orders
    reconstructed.push(parts[6].trim()); // last_purchase
    reconstructed.push(parts[7].trim()); // segment
    reconstructed.push(parts[8].trim()); // products_purchased
    ...
    // Too many fields, something else is wrong
    return null;
  }
  
  // Now we should have exactly 8 fields
...
async function processCsvFile(filePath) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      if (!values) {
        console.warn(`Skipping row ${i + 1}: could not parse line`);
        continue;
      }

      // We know the indices from the header:
      //   0: first_name
      //   1: last_name
      //   2: email
      //   3: total_spend
      ...
      //   5: last_purchase
      //   6: segment
      //   7: products_purchased
      const emailIndex = 2;
      const firstNameIndex = 0;
      const lastNameIndex = 1;
      const productIndex = 7; // products_purchased

      // Ensure we have enough values
      if (values.length < 8) {
        console.warn(`Skipping row ${i + 1}: insufficient columns after parsing (${values.length} < 8)`);
        continue;
      }
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: node process-user-purchases-hardcoded.cjs <csv-file-path>');
